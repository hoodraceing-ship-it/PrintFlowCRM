...
<div class='box'><b>Customer notes</b><br>{html_lib.escape(row['notes'] or '—').replace(chr(10),'<br>')}</div><div class='footer'>Packed by: ____________________ &nbsp;&nbsp; Date: __________</div></body></html>"""
    ...
    def show_settings(self):
        ...
        ttk.Label(ai,text="The configured OpenAI model powers AI-ranked Model Finder and, after one-time consent in Messenger, full Marketplace chat translation and context-aware sales replies. Completed chat translations are saved locally and reused without another API charge until that conversation changes; Refresh Reply deliberately generates a new result. Free Usage Preferred uses gpt-5.4-mini. Lowest Paid Cost uses gpt-5.6-luna. Higher Quality uses gpt-5.4.",style="Card.TLabel",wraplength=780,justify="left").grid(row=4,column=0,columnspan=3,sticky="w",pady=(4,7))
